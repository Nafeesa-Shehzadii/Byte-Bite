import { useListAvailableOrders, useListDriverOrders, useAcceptOrder, useUpdateOrderStatus, getListAvailableOrdersQueryKey, getListDriverOrdersQueryKey } from "@workspace/api-client-react";
...
export default function DriverDashboard() {
  ...
  const { data: availableOrders, isLoading: availLoading } = useListAvailableOrders({ query: { refetchInterval: 3000, queryKey: getListAvailableOrdersQueryKey() }});
  const { data: myOrders, isLoading: myLoading } = useListDriverOrders({ driverName }, { query: { enabled: !!driverName, refetchInterval: 3000, queryKey: getListDriverOrdersQueryKey({ driverName }) }});
  ...
  if (availLoading || myLoading) {
    return <div className="space-y-6 max-w-4xl mx-auto">
      {[1,2,3].map(i => <Skeleton key={i} className="h-48 w-full bg-[#111111] rounded-3xl border border-white/10" />)}
    </div>;
  }
    ...
    <div className="space-y-12 max-w-4xl mx-auto pb-20">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 border-b border-white/10 pb-6">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 bg-primary/10 rounded-2xl border border-primary/20 flex items-center justify-center shadow-[0_0_30px_rgba(230,57,70,0.15)]">
            <Navigation className="w-8 h-8 text-primary" />
          </div>
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-white">Driver Terminal</h1>
            <p className="text-zinc-400 font-mono mt-1">Callsign: <span className="text-primary font-bold">{driverName}</span></p>
          </div>
        </div>
      </div>
          ...
          <AnimatePresence>
            {activeMyOrders.map(order => (
              <motion.div
                ...
                className="bg-[#111111] backdrop-blur-2xl border border-primary/30 shadow-[0_0_40px_rgba(230,57,70,0.1)] rounded-3xl p-6 sm:p-8 relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 bg-primary text-white px-6 py-2 text-sm font-black uppercase tracking-wider rounded-bl-2xl shadow-lg">
                  {order.status}
                </div>
                ...
                <div className="space-y-6 pt-4 sm:pt-0">
                  ...
                  <div className="bg-[#0A0A0A] rounded-2xl p-5 border border-white/5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-5">
          <AnimatePresence>
            {availableOrders?.map(order => (
              <motion.div
                ...
                className="bg-[#111111] backdrop-blur-xl border border-white/10 hover:border-primary/50 hover:bg-[#111111]/80 transition-all rounded-3xl p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-6 shadow-xl group"
              >
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <h3 className="font-bold text-2xl text-white group-hover:text-primary transition-colors">{order.restaurantName}</h3>
                ...
                <Button 
                  ...
                  variant="outline"
                  className="border-white/20 text-white hover:bg-primary hover:text-white hover:border-primary transition-all shrink-0 font-bold px-8 py-6 text-base shadow-lg"
                >
                  Accept Pickup
                </Button>
              </motion.div>
          ...
          {(!availableOrders || availableOrders.length === 0) && (
            <div className="text-center py-20 px-4 border-2 border-dashed border-white/5 rounded-3xl bg-[#111111]/50">
