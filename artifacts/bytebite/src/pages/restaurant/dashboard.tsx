  ...
  if (sumLoading || ordLoading) {
    return <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[1,2,3].map(i => <Skeleton key={i} className="h-32 bg-[#111111] rounded-3xl border border-white/10" />)}
      </div>
      <Skeleton className="h-[600px] w-full bg-[#111111] rounded-3xl border border-white/10" />
    </div>;
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-[#111111]/80 backdrop-blur-xl border-white/10 shadow-2xl relative overflow-hidden rounded-3xl">
          ...
          <CardContent className="p-8 flex flex-col justify-center relative z-10">
            ...
            <div className="flex items-center gap-2 text-4xl font-black text-primary tracking-tight">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <AnimatePresence>
            {activeOrders.map(order => (
              <motion.div
                ...
                className="bg-[#111111] backdrop-blur-2xl border border-white/10 hover:border-primary/30 transition-colors shadow-2xl rounded-3xl p-6 md:p-8 flex flex-col gap-6"
              >
                <div className="flex justify-between items-start gap-4">
                  ...
                  <div className="font-mono text-xl font-bold text-primary bg-primary/10 px-4 py-2 rounded-xl">
                    ${(order.total / 100).toFixed(2)}
                  </div>
                </div>

                <div className="bg-[#0A0A0A] rounded-2xl p-5 border border-white/5 space-y-3 shadow-inner">
                  {order.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between text-base border-b border-white/5 last:border-0 pb-3 last:pb-0">
                      <span className="font-medium text-zinc-300">
                        <span className="inline-block w-8 text-primary font-bold bg-primary/10 text-center rounded mr-3 py-0.5 text-sm">{item.quantity}x</span> 
                        {item.name}
                      </span>
                    </div>
                  ))}
          ...
          {activeOrders.length === 0 && (
            <div className="col-span-1 xl:col-span-2 text-center py-24 px-4 border-2 border-dashed border-white/10 rounded-3xl bg-[#111111]/20">
