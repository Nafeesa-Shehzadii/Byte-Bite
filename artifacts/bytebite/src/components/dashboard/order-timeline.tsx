      ...
      <div className="space-y-0">
        {recent.map((o, i) => (
          <div key={o.id} className="flex gap-3 relative">
            {i < recent.length - 1 && <div className="absolute left-[7px] top-5 bottom-0 w-0.5 bg-gray-100" />}
            <div className={`w-3.5 h-3.5 rounded-full mt-1 shrink-0 ${STATUS_DOT[o.status] || "bg-gray-300"}`} />
            <div className="flex-1 pb-4 min-w-0">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm text-gray-900 font-medium truncate">#{o.id} - {o.customerName}</p>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <span className="text-[10px] text-gray-400">{formatDistanceToNow(new Date(o.createdAt), { addSuffix: true })}</span>
              </div>
            </div>
          </div>
        ))}
